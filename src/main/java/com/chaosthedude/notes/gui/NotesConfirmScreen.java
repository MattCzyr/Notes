...
public class NotesConfirmScreen extends ConfirmScreen {
	...
	protected void init() {
		super.init();
		clearWidgets();
		addRenderableWidget(new NotesButton(this.width / 2 - 155, height / 6 + 96, 150, 20, new TranslatableComponent("gui.yes"), (button) -> {
			callbackFunction.accept(true);
		}));
		addRenderableWidget(new NotesButton(this.width / 2 - 155 + 160, height / 6 + 96, 150, 20, new TranslatableComponent("gui.no"), (button) -> {
